...
app.post("/consultas", async (req, res) => {
  const { paciente_id, data_consulta, medico } = req.body || {};
  ...
  // Garante que a primeira letra da observação seja maiúscula
  if (observacao && typeof observacao === "string" && observacao.trim().length > 0) {
    observacao = observacao.trim();
    observacao = observacao.charAt(0).toUpperCase() + observacao.slice(1);
  } else {
    observacao = null;
  }
  ...
  try {
    const { rows } = await pool.query(
      ...
      [Number(paciente_id), data_consulta, medico || null, observacao]
    );
    res.status(201).json(rows[0]);
  } catch (e) {
    console.error(e);
